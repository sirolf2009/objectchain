...
class TestBlockchain {
	...
	def void testBranching() {
		...
		val side1 = new Block(new BlockHeader(genesis.hash(kryo), new Hash(newArrayOfSize(0)), new Date(), new BigInteger("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF", 16), 4), new TreeSet())
		val side2 = new Block(new BlockHeader(side1.hash(kryo), new Hash(newArrayOfSize(0)), new Date(), new BigInteger("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF", 16), 5), new TreeSet())
		...
		val side3 = new Block(new BlockHeader(side2.hash(kryo), new Hash(newArrayOfSize(0)), new Date(), new BigInteger("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF", 16), 6), new TreeSet())
		branch.blocks.add(side3)
		Assert.assertEquals(4, blockchain.totalBranchLength(branch))
		Assert.assertFalse(blockchain.isBranchLonger(branch))
		
		val side4 = new Block(new BlockHeader(side3.hash(kryo), new Hash(newArrayOfSize(0)), new Date(), new BigInteger("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF", 16), 7), new TreeSet())
